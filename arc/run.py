...
# ============================================================
# TRAIN CFG  (dernier polishing run vers ~1000 epochs cumulés)
# ============================================================
...
TRAIN_CFG = dict(
    ...
    # but: polishing fin sans casser le bon équilibre atteint
    change_weight=4.0,
    unchanged_weight=1.20,
    eos_weight=1.2,
    ...
    resume_from="/gpfs/users/imagroune/work/ierm/arc/checkpoints_v4_4/best_ema.pt",
)


def apply_curriculum_polish(epoch, cfg, model=None):
    """
    Dernier run de polishing:
      - T=2 dès le début
      - LR bas
      - légère montée d'augmentation, sans changement brutal
      - objectif: convertir une partie du near_solved en solved
    """

    # ── Phase 1 : polishing conservateur ─────────────────────
    if epoch < 80:
        cfg["T_per_step"] = 2
        cfg["aug_prob"] = 0.70
        cfg["color_dropout_prob"] = 0.0
        cfg["equivariance_lambda"] = 0.0
        cfg["hard_mining_factor"] = 1.10
        cfg["residual_alpha"] = 0.40
        cfg["support_verify_lambda"] = 0.05
        cfg["loso_lambda"] = 0.20
        cfg["change_weight"] = 4.0
        cfg["unchanged_weight"] = 1.20
        cfg["eos_weight"] = 1.2
        if model is not None:
            model.psi.T_psi = 2

    # ── Phase 2 : légère pression supplémentaire ─────────────
    elif epoch < 180:
        cfg["T_per_step"] = 2
        cfg["aug_prob"] = 0.80
        cfg["color_dropout_prob"] = 0.05
        cfg["equivariance_lambda"] = 0.0
        cfg["hard_mining_factor"] = 1.15
        cfg["residual_alpha"] = 0.40
        cfg["support_verify_lambda"] = 0.05
        cfg["loso_lambda"] = 0.20
        cfg["change_weight"] = 4.05
        cfg["unchanged_weight"] = 1.18
        cfg["eos_weight"] = 1.2
        if model is not None:
            model.psi.T_psi = 2

    # ── Phase 3 : finition stable, pas agressive ─────────────
    else:
        cfg["T_per_step"] = 2
        cfg["aug_prob"] = 0.85
        cfg["color_dropout_prob"] = 0.05
        ...
        cfg["support_verify_lambda"] = 0.05
        cfg["loso_lambda"] = 0.20
        cfg["change_weight"] = 4.10
        cfg["unchanged_weight"] = 1.15
        cfg["eos_weight"] = 1.2
        if model is not None:
            model.psi.T_psi = 2
...
def main():
    ...
    output_dir = Path("~/work/ierm/runs/arc_lmhead_polish_v1").expanduser()
    ...
    train_mod.apply_curriculum = apply_curriculum_polish
    ...
    print(f"[RUN] resume_from={TRAIN_CFG['resume_from']}", flush=True)
    print(
        f"[RUN] lr={TRAIN_CFG['lr']} warmup_steps={TRAIN_CFG['warmup_steps']} "
        f"T_per_step={TRAIN_CFG['T_per_step']} "
